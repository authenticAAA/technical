...
- **豊富なテクニカル指標**:
  - トレンド系: SMA(20/50/200)、EMA、MACD、ADX(+DI/-DI)、パラボリックSAR、一目均衡表
  - オシレーター系: RSI、ストキャスティクス、CCI、Williams %R、MFI、ROC
  - ボラティリティ系: ボリンジャーバンド、ATR
  - 出来高系: OBV、MFI
- **フォーメーション分析**: スイング高値・安値（ピボット）から以下を自動検出
  - サポート / レジスタンスライン
  - トレンド構造（高値・安値の切り上げ/切り下げ）
  - ダブルトップ / ダブルボトム
  - ヘッドアンドショルダー / 逆ヘッドアンドショルダー
  - 直近レンジのブレイクアウト / ブレイクダウン
- **指標の表示切替**: 価格チャートに重ねる指標・別パネルで表示する指標を**ドロップダウンリスト**で自由に選択
- **売買シグナル**: ゴールデン/デッドクロス、RSI 過熱、MACD/ADX/一目均衡表など多数の指標を総合スコア化し `BUY / SELL / NEUTRAL` を判定
